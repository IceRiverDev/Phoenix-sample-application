...
import com.cz.phoenix.config.EmbeddedKafka;
import com.cz.phoenix.config.KafkaSseConsumer;
import com.cz.phoenix.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class PhoenixApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/phoenix-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/phoenix-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/phoenix-application-kafka/unregister"));
                return;
            }
        }
